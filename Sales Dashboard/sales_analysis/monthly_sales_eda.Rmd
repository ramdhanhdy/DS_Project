...
This analysis delves into monthly sales data spanning from 2019 to 2021, comprising essential variables like Total Revenue, Total Quantity Sold, Total Budget, Sales Person, Supervisor, Manager, Product Name, and Product Category. The primary aim is to explore the intricate relationship between budget allocation and revenue fluctuations over time.

Among the highlight of this analysis is the correlation assessments with distinct time lags (zero-lag to three-month lags) to ascertain whether budget adjustments exert an immediate or delayed impact on revenue. Furthermore, the study investigates correlations between revenue and other variables, such as salesperson performance and product categories.
...
# Get tidy ANOVA results and display the ANOVA summary table
tidy_anova <- revenue_anova %>% 
  tidy() %>%
  kable(format = "markdown", col.names = c("Term", "Df", "Sum Sq", "Mean Sq", "F value", "Pr(>F)"))
```
...
The previous statistical finding comes with an assumption that the impact of budget revenue is
